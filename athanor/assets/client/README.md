...
```bash
athanor client up --host-ip <host> --host-discover-port <port>
```

Or as a systemd service:

```bash
athanor client up --service --host-ip <host> --host-discover-port <port>
```

## Configuration

Environment variables are read from `~/.local/share/athanor/client/.env`:
