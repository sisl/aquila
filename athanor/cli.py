...
HOST_SERVICE_NAME = "athanor"
CLIENT_SERVICE_NAME = "athanor-client"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="athanor",
        description="Athanor — GPU inference cluster manager",
...
def run_clean(remove_docker: bool = False, assume_yes: bool = False) -> None:
    ...
    print("athanor clean will remove:")
    for p in targets:
        print(f"  - {p}")
    ...
    if installed_units:
        ...
        for u in installed_units:
            print(f"    {u}")
        print("  Remove them with `athanor host down` / `client down`.")
...
def _clean_docker() -> None:
    ...
    try:
        out = run(
            [docker, "ps", "-aq", "--filter", "label=athanor.managed=true"],
            capture=True,
        )
        ids = out.split()
        if ids:
    ...
    try:
        ...
        images = [
            ...
            or line.startswith("athanor/local:")
        ]
        if images:
...
def ensure_runtime_dir(kind: str) -> Path:
    base_dir = Path(os.environ.get("XDG_DATA_HOME", Path.home() / ".local" / "share"))
    runtime_dir = base_dir / "athanor" / kind
...
def copy_assets(kind: str, dest: Path) -> None:
    src_root = resources.files("athanor.assets") / kind
...
def copy_assets_subdir(kind: str, subdir: str, dest: Path) -> None:
    src_root = resources.files("athanor.assets") / kind / subdir
    if not src_root.is_dir():
