...
```bash
vllm-cluster-manager client up --host-ip <host> --host-discover-port <port>
```

Or as a systemd service:

```bash
vllm-cluster-manager client up --service --host-ip <host> --host-discover-port <port>
```

## Configuration

Environment variables are read from `~/.local/share/vllm_cluster_manager/client/.env`:
