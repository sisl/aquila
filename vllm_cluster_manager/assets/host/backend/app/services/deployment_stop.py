...
async def stop_deployment_internal(
    ...
    *,
    ...
    (used by expiry/drain so a dead node can't keep a deployment alive
    forever); otherwise the error propagates. The caller commits.
    """
    if node is not None:
    ...
    deployment.expires_at = None  # any stop ends the serve countdown
